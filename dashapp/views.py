...
from dashapp.generalUtil import returnStatusValue,monthDataToLists,rolesAndCounters
...
def dashPageShowInflight_old(request):

    try:

        select = ("SELECT projectName,category,projectDeliveryManager,dealValue,overallStatus,status1 FROM projectdetails2;")
        selectEnt = ("SELECT category,count(category) FROM projectdetails2 where category = 'ENT';")
        selectSp = ("SELECT category,count(category) FROM projectdetails2 where category = 'SP';")
        scheduleStatus = ("SELECT projectName,scheduleStatus FROM projectdetails2;")

        closed = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Closed';")
        inProgres = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery In-Progress';")
        notStarted = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Not-Started';")

        allVal = selectDB(select)
        ent = selectDB(selectEnt)
        sp =  selectDB(selectSp)
        schedule = selectDB(scheduleStatus)
        
        clo = selectDB(closed)
        pro = selectDB(inProgres)
        notStart = selectDB(notStarted)

        sYellow = list(clo.values())[0][0]
        sred = list(notStart.values())[0][0]
        sGreen = list(pro.values())[0][0]
        ...
        for k,v in schedule.items():
            if v == 'Red':
                ...
                sYellow += 1
        '''
        for k,v in allVal.items():
            projectsList.append(k)
            if(v[0] == 'ENT' and v[4] != 'Delivery Closed'):
                entCount += 1
            elif(v[0] == 'SP' and v[4] != 'Delivery Closed'):
                spCount += 1
            category.append(v[0])
            dm.append(v[1])
            deal.append(v[2])
            overallStatus.append(v[3])
            status1.append(v[4])
        ...
        for status in overallStatus:
            ...
            #status == 'Green' and s1 != 'Delivery Closed'
        red = 0
        yellow = 0
        green = 0
        for (status,s1) in zip(overallStatus,status1):
            if status == 'Gray' and s1 != 'Delivery Closed':
                red += 1
            elif status == 'Yellow' and s1 != 'Delivery Closed':
                yellow += 1
            elif status == 'Green' and s1 != 'Delivery Closed':
                green += 1
        #print(statusList)
        zippedList = zip(projectsList,category,dm,deal,status1,statusList)

        #("SELECT availabilityMonth5 FROM availabilityTracker  WHERE CHAR_LENGTH(nce)>2 and reportingManager = \'"+str(manager)+"\';")
    except:
        return render(request,'mainPage_inflight.html')

    #entCount = json.dumps(entCount)
    #spCount = json.dumps(spCount)
    print(sred)
    print(sYellow)
    print(sGreen)
    return render(request,'mainPage_inflight.html',{'zippedList':zippedList,'entCount':entCount,'spCount':spCount,'green':green, 'yellow':yellow,'red':red,'sred':sred,'sGreen':sGreen,'sYellow':sYellow})
    
def dashPageShowInflight(request):
    ...
    try:
        ...
        closedent = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Closed' and category = 'ENT';")
        inProgresent = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery In-Progress' and category = 'ENT';")
        notStartedent = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Not-Started' and category = 'ENT';")
        
        closedsp = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Closed'and category = 'SP';")
        inProgressp = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery In-Progress' and category = 'SP';")
        notStartedsp = ("SELECT status1, count(status1) FROM projectdetails2 where status1 = 'Delivery Not-Started' and category = 'SP';")
        ...
        cloent = selectDB(closedent)
        proent = selectDB(inProgresent)
        notStartent = selectDB(notStartedent)
        
        closp = selectDB(closedsp)
        prosp = selectDB(inProgressp)
        notStartsp = selectDB(notStartedsp)

        sYellow = list(clo.values())[0][0]
        sred = list(notStart.values())[0][0]
        sGreen = list(pro.values())[0][0]
        
        entsYellow = list(cloent.values())[0][0]
        entsRed = list(notStartent.values())[0][0]
        entsGreen = list(proent.values())[0][0]
        
        spsyellow = list(closp.values())[0][0]
        spsred = list(notStartsp.values())[0][0]
        spsgreen = list(prosp.values())[0][0]
        ...
        entprojectsList = list()
        entcategory = list()
        entdm = list()
        entdeal = list()
        entoverallStatus = list()
        entstatus1 = list()
        ...
        for k,v in allVal.items():
            if v[0] == 'ENT':
                entprojectsList.append(k)
                entcategory.append(v[0])
                entdm.append(v[1])
                entdeal.append(v[2])
                entoverallStatus.append(v[3])
                entstatus1.append(v[4])
            elif v[0] == 'SP':
                spprojectsList.append(k)
                spcategory.append(v[0])
                spdm.append(v[1])
                spdeal.append(v[2])
                spoverallStatus.append(v[3])
                spstatus1.append(v[4])
        ...
        for (status,s1) in zip(entoverallStatus,entstatus1):
            if status == 'Gray' and s1 != 'Delivery Closed':
                entstatusList.append('Red')
                entred += 1
            elif status == 'Yellow' and s1 != 'Delivery Closed':
                entstatusList.append(status)
                entyellow += 1
            elif status == 'Green' and s1 != 'Delivery Closed':
                entstatusList.append(status)
                entgreen += 1
                
        for (status,s1) in zip(spoverallStatus,spstatus1):
            if status == 'Gray' and s1 != 'Delivery Closed':
                spstatusList.append('Red')
                spred += 1
            elif status == 'Yellow' and s1 != 'Delivery Closed':
                spstatusList.append(status)
                spyellow += 1
            elif status == 'Green' and s1 != 'Delivery Closed':
                spstatusList.append(status)
                spgreen += 1
        ...
        #print(statusList)
        zippedList = zip(projectsList,category,dm,deal,status1,statusList)
        zippedListEnt = zip(entprojectsList,entcategory,entdm,entdeal,entstatus1,entstatusList)
        zippedListSp = zip(spprojectsList,spcategory,spdm,spdeal,spstatus1,spstatusList)
    ...
    #print(sYellow)
    #print(sGreen)
    return render(request,'mainPage_inflight.html',{'zippedList':zippedList,'entCount':entCount,'spCount':spCount,'green':green, 'yellow':yellow,'red':red,'sred':sred,'sGreen':sGreen,'sYellow':sYellow,'zippedListEnt':zippedListEnt,'zippedListSp':zippedListSp,'entred':entred,'entyellow':entyellow,'entgreen':entgreen,'spred':spred,'spyellow':spyellow,'spgreen':spgreen,'spsred':spsred,'spsyellow':spsyellow,'spsgreen':spsgreen,'entsred':entsRed,'entsyellow':entsYellow,'entsgreen':entsGreen})
...
def skillsSearch(request):
    if request.method == "POST":
        ...
        for value in selectedValues:
            name.append(value[0])
            project.append(value[1])
            manager.append(value[2])
            entOrSp.append(value[3])
        
        zippedList = zip(cecList,name,project,manager,entOrSp)
        
        selectedValues = [selectedValues]
        return render(request, 'skillsSearchPage.html',{'selectedValues':selectedValues,'entSpFlag':entSpFlag,'zippedList':zippedList})
    ...
    if "GET" == request.method:
        ...
        for i,row in enumerate(worksheet.iter_rows()):
            ...
            if(len(rows) >= 1):
                ...
                updateParentQuery = ("UPDATE Dashboard_Info set category = \'"+str(row_data[4])+"\', dmName = \'"+str(row_data[23])+"\', completionZone = \'"+str(row_data[24])+"\', projectStatus = \'"+str(row_data[14])+"\' WHERE projectName = \'"+str(row_data[0])+"\';")
...
def uploadInMasterProjectsDB(request):
    if "GET" == request.method:
        ...
        for i,row in enumerate(worksheet.iter_rows()):
            ...
            tempQuery = ("SELECT projectName from migration_Projects where projectName = \'"+str(row_data[1])+"\' and projectBookingYear = \'"+str(row_data[2])+"\';")
            rows = listDB(tempQuery)
            deletedRows = 0
            if(len(rows) >= 1):
                deleteQuery = ("DELETE from migration_Projects where projectName = \'"+str(row_data[1])+"\' and projectBookingYear = \'"+str(row_data[2])+"\';")
                deletedRows = deleteDB(deleteQuery)
                #delete from parent
...
def uploadavailabilityTracker(request):
    if "GET" == request.method:
        ...
        for i,row in enumerate(worksheet.iter_rows()):
            ...
            selectResource = ("SELECT cecID FROM Resources where cecID = \'"+str(row_data[3])+"\';")
            resRows = selectDB(selectResource)
            if len(resRows) >=1:
                    updateQuery = ("UPDATE Resources set assignedProject = \'"+str(row_data[6])+"\', badge = \'"+str(row_data[5])+"\' where cecID = \'"+str(row_data[3])+"\';")
                    print(updateQuery)
                    #print(updateQuery)
                    updateDB(updateQuery)
            else:
                insertRes = ("INSERT INTO Resources(cecID,name,projectRole,email,badge,assignedProject) values (%s,%s,%s,%s,%s,%s);")
                resourceList = list()
                print(row_data[2])
                if isinstance(row_data[2], int):
                    continue
                elif len(row_data[2]) <= 2:
                    print(row_data[1])
                    print(row_data[2])
                    continue
                else:
                    resourceList.append(row_data[3])
                    resourceList.append(row_data[2])
                    resourceList.append(row_data[4])
                    resourceList.append(row_data[3]+"@cisco.com")
                    resourceList.append(row_data[5])
                    resourceList.append(row_data[6])
                    res = insertDB(insertRes,resourceList)
...
def showprojectDetails_new(request, projectName):
    ...
    selectProjQuery2 = ("SELECT * FROM migration_Projects  WHERE projectName = \'"+str(projectName)+"\' order by projectBookingYear desc limit 1;")
    ...
    try:
        ...
        try:
            ...
            averageCBU = "NA"
        try:
            completionStatus = projectData[24]
        except:
            completionStatus = "NA"
        ...
        for i in range(13,20):
            if projectData[i] == 'Green':
                statusList.append(3)
                statusList1.append(projectData[i])
...
def resourceDetails1(request, cecID):
    #cecID
    try:
        selectResourceQuery = ("SELECT cecId,name,email,assignedProject,entOrSp,manager,inclination,badge,projectRole FROM Resources  WHERE cecID = \'"+str(cecID)+"\';")
    ...
    'Qos','virtualization','Security','Nexus Hardware','VX Lan','SD WAN','DNAC','Ipv6','Meraki','SASE','SDN','MultiCloud']
        ...
        return render(request,'resourceDetails.html',{'devnet':certList[0],'devnetDate':certList[1],'specia':certList[2],'speciaDate':certList[3],'manager':manager,'project':rList[2],
        'entsp':rList[3],'incli':incli,'name':rList[0],'entInfo':entInfo,'spInfo':spInfo,'migInfo':migInfo,'scriptInfo':scriptInfo
        ,'ccieInfo':ccieInfo,'compPlatInfo':compPlatInfo,'softInfo':softInfo,'badge':rList[6],'role':rList[7]})
...
def ResourcesInfo1(request):
    ...
    selJsonXml = ("SELECT count(jsonXml) FROM scripting  WHERE jsonXml =\'"+str(1)+"\';")
    selNetConf = ("SELECT count(netconfRestconfYang) FROM scripting  WHERE netconfRestconfYang =\'"+str(1)+"\';")
    selectManagerQuery = ("SELECT distinct(reportingManager) FROM availabilityTracker;")
    ...
    netConfCount = list(selectDB(selNetConf).keys())[0]
    managerRes = selectDB(selectManagerQuery)
    managersList = list(selectDB(selectManagerQuery).keys())
    ...
    for manager in managersList:
        countQuery = ("SELECT count(nce) FROM availabilityTracker  WHERE reportingManager = \'"+str(manager)+"\';")
        empCount.append(list(selectDB(countQuery))[0])
...
def showTelemetryAvailabilityTracker(request):
    managersQ = ("SELECT distinct(reportingManager) FROM availabilityTracker;")
    selectManagerQuery = ("SELECT reportingManager, nce FROM availabilityTracker  WHERE CHAR_LENGTH(nce)<=2;")
    selectManagers = selectDB(selectManagerQuery)
    print(list(selectDB(managersQ).keys()))

    #managers = list()
    managers = list(selectDB(managersQ).keys())
    ...
    for manager in managers:
        countQuery = ("SELECT count(nce) FROM availabilityTracker  WHERE reportingManager = \'"+str(manager)+"\';")
        reporteesCount.append(list(selectDB(countQuery))[0])

    #print(reporteesCount)
    #print(managers)
    ...
    for manager in managers:
        ...
        for k,v in temp.items():
            if v[1] is None or v[1] == '0':
                ...
                if m1 > 0:
                    m1count += 1
            if v[2] is None or v[2] == '0':
                m2 += 0
            else:
                m2 += round(float(v[2]) *100,2)
                if m2 > 0:
                    m2count += 1
            if v[3] is None or v[3] == '0':
                m3 += 0
            else:
                m3 += round(float(v[3]) *100,2)
                if m3 > 0:
                    m3count += 1
            if v[4] is None or v[4] == '0':
                m4 += 0
            else:
                m4 += round(float(v[4]) *100,2)
                if m4 > 0:
                    m4count += 1
            if v[5] is None or v[5] == '0':
                m5 += 0
            else:
        ...
        #print("************")
        #print(m1count)
        tempCount = [m1count,m2count,m3count,m4count,m5count]
        tempList = [m1,m2,m3,m4,m5]
        bwDetails.append(tempList)
        countDetails.append(tempCount)
    mon1 = list()
    mon2 = list()
    mon3 = list()
    mon4 = list()
    ...
    c4 = json.dumps(c4)
    c5 = json.dumps(c5)
    reportees = reporteesCount
    reporteesCount = json.dumps(reporteesCount)
    
    mgrRepzip = zip(minfo,reportees)
    mgrBwCountZip = zip(minfo,m1Bw,m2Bw,count1,count2)
    mgrBwCountZip2 = zip(minfo,m3Bw,m4Bw,m5Bw,count3,count4,count5)


    return render(request, 'showTelemetryAvailabilityTracker.html',{'managers':managers,'mon1':mon1,'mon2':mon2,'mon3':mon3,'mon4':mon4,'mon5':mon5,'mon11':mon11,'mon22':mon22,'mon33':mon33,'mon44':mon44,'mon55':mon55,'reporteesCount':reporteesCount,'countDetails':countDetails,'c1':c1,'c2':c2,'c3':c3,'c4':c4,'c5':c5,'month1':month1,'month2':month2,'month3':month3,'month4':month4,'month5':month5,'minfo':minfo,'mgrRepzip':mgrRepzip,'mgrBwCountZip':mgrBwCountZip,'mgrBwCountZip2':mgrBwCountZip2})
    
    
    
    
def showprojectDetails_newTemp(request, projectName):
    selectProjQuery = ("SELECT * FROM projectdetails2  WHERE projectName = \'"+str(projectName)+"\';")
    selectProjQuery2 = ("SELECT * FROM migration_Projects  WHERE projectName = \'"+str(projectName)+"\';")
    result = selectDBParam(selectProjQuery,projectName)
    result2 = selectDBParam(selectProjQuery2,projectName)
    ...
    return render(request, 'projectDetails.html', {'numberOfSitesPlanned':65,'numOfSitesMigrated':40,
        'pID': projectData[7], 'projectName':projectName, 'summary':projectData[21], 'nceIndia':projectData[2], 
        'nceOther':projectData[3],'avgCBU':avgCBU, 'region': region, 'endDate': projectData[1], 
        'comments':projectData[22],'techno':projectData[5],'bookingYear':data2[1],'eFY':data2[2],'lead':data2[3],
        'mTrack':data2[6],'osVer':data2[7],'dMigrated':data2[16],'wExecuted':data2[17],'wSuccess':wSuccess,
        'pStatus':data2[19],'cVal':cVal,'custSenti':projectData[13],'oVal':oVal,'oStatus':projectData[14],'aVal':aVal,
        'aStatus': projectData[18], 'sStatus':projectData[15],'sVal':sVal,'qVal':qVal, 'qStatus':projectData[16],
        'dManager':projectData[23],'dStatus':projectData[24],'pCompletionLevel':projectData[25],
        'dataList':dataList,'statusLabels':statusLabels,'statusList':statusList,'totalNCE':totalNCE,
        'averageCBU':averageCBU})
        

def skillsTele(request):

    managersQ = ("SELECT distinct(reportingManager) FROM availabilitytracker;")

    month1Availability = ("SELECT nce,reportingManager,availabilityMonth1,role1 FROM availabilityTracker  WHERE availabilityMonth1 > '0'")
    month2Availability = ("SELECT nce,reportingManager,availabilityMonth2,role1 FROM availabilityTracker  WHERE availabilityMonth2 > '0'")
    month3Availability = ("SELECT nce,reportingManager,availabilityMonth3,role1 FROM availabilityTracker  WHERE availabilityMonth3 > '0'")
    month4Availability = ("SELECT nce,reportingManager,availabilityMonth4,role1 FROM availabilityTracker  WHERE availabilityMonth4 > '0'")
    month5Availability = ("SELECT nce,reportingManager,availabilityMonth5,role1 FROM availabilityTracker  WHERE availabilityMonth5 > '0'")

    m1Ava = selectDB(month1Availability)
    m2Ava = selectDB(month2Availability)
    m3Ava = selectDB(month3Availability)
    m4Ava = selectDB(month4Availability)
    m5Ava = selectDB(month5Availability)

    m1empList,m1empMgr,m1empBw,m1Role,m1Roles,m1RoleCounter = list(),list(),list(),list(),list(),list()
    m2empList,m2empMgr,m2empBw,m2Role,m2Roles,m2RoleCounter = list(),list(),list(),list(),list(),list()
    m3empList,m3empMgr,m3empBw,m3Role,m3Roles,m3RoleCounter = list(),list(),list(),list(),list(),list()
    m4empList,m4empMgr,m4empBw,m4Role,m4Roles,m4RoleCounter = list(),list(),list(),list(),list(),list()
    m5empList,m5empMgr,m5empBw,m5Role,m5Roles,m5RoleCounter = list(),list(),list(),list(),list(),list()
    

    m1empList,m1empMgr,m1empBw,m1Role = monthDataToLists(m1Ava)
    m2empList,m2empMgr,m2empBw,m2Role = monthDataToLists(m2Ava) 
    m3empList,m3empMgr,m3empBw,m3Role = monthDataToLists(m3Ava)
    m4empList,m4empMgr,m4empBw,m4Role = monthDataToLists(m4Ava)
    m5empList,m5empMgr,m5empBw,m5Role = monthDataToLists(m5Ava) 

    m1RoleDictCounter = dict(Counter(m1Role))
    m2RoleDictCounter = dict(Counter(m2Role))
    m3RoleDictCounter = dict(Counter(m3Role))
    m4RoleDictCounter = dict(Counter(m4Role))
    m5RoleDictCounter = dict(Counter(m5Role))

    m1Roles,m1RoleCounter = rolesAndCounters(m1RoleDictCounter)
    m2Roles,m2RoleCounter = rolesAndCounters(m2RoleDictCounter)
    m3Roles,m3RoleCounter = rolesAndCounters(m3RoleDictCounter)
    m4Roles,m4RoleCounter = rolesAndCounters(m4RoleDictCounter)
    m5Roles,m5RoleCounter = rolesAndCounters(m5RoleDictCounter)
    ...
    month1 = monthsTracker[0]
    month2 = monthsTracker[1]
    month3 = monthsTracker[2]
    month4 = monthsTracker[3]
    month5 = monthsTracker[4]

    m1Zip = zip(m1empList,m1empMgr,m1empBw)
    m2Zip = zip(m2empList,m2empMgr,m2empBw)
    m3Zip = zip(m3empList,m3empMgr,m3empBw)
    m4Zip = zip(m4empList,m4empMgr,m4empBw)
    m5Zip = zip(m5empList,m5empMgr,m5empBw)
    
    m1RoleZip = zip(m1Roles,m1RoleCounter)
    m2RoleZip = zip(m2Roles,m2RoleCounter)
    m3RoleZip = zip(m3Roles,m3RoleCounter)
    m4RoleZip = zip(m4Roles,m4RoleCounter)
    m5RoleZip = zip(m5Roles,m5RoleCounter)

    m1empMgr = list(set(m1empMgr))
    m2empMgr = list(set(m2empMgr))
    m3empMgr = list(set(m3empMgr))
    m4empMgr = list(set(m4empMgr))
    m5empMgr = list(set(m5empMgr))

    return render(request, 'TeamAvailabilityTele.html',{'month1':month1,'m1Count':len(m1Ava),'m1Zip':m1Zip,'m1RoleZip':m1RoleZip,'month2':month2,'m2Count':len(m2Ava),'m2Zip':m2Zip,'m2RoleZip':m2RoleZip,'month3':month3,'m3Count':len(m3Ava),'m3Zip':m3Zip,'m3RoleZip':m3RoleZip,'month4':month4,'m4Count':len(m4Ava),'m4Zip':m4Zip,'m4RoleZip':m4RoleZip,'month5':month5,'m5Count':len(m5Ava),'m5Zip':m5Zip,'m5RoleZip':m5RoleZip,      'm1empMgr':m1empMgr,'m2empMgr':m2empMgr,'m3empMgr':m3empMgr,'m4empMgr':m4empMgr,'m5empMgr':m5empMgr})